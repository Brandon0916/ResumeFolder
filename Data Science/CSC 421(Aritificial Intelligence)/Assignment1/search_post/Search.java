...
public class Search {
	...
	private String TreeSearchDepthLimited(Frontier frontier, int limit) {
		//TODO
		//just use the given method and variable from above function
		cnt = 0; 
		node_list = new ArrayList<Node>();
		
		initialNode = MakeNode(problem.initialState); 
		//Map<Object,Double> explored = new HashMap<Object,Double>();
		node_list.add( initialNode );
		frontier.insert(initialNode);
		while(true){
			if(frontier.isEmpty()){
			...
			//as there is a depth attribute in Node.java
			if( node.depth<limit){
	...
	private String GraphSearchDepthLimited(Frontier frontier, int limit) {
		//TODO
		//same idea with the tree and same with A1 Q&A create by Alex
		cnt = 0; 
        node_list = new ArrayList<Node>();
